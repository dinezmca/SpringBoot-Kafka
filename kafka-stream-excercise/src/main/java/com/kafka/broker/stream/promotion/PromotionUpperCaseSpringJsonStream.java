...
public class PromotionUpperCaseSpringJsonStream {
	...
	public KStream<String, PromotionMessage> build(StreamsBuilder builder) {
		...
		JsonSerde<PromotionMessage> jsonserdes = new JsonSerde<>(PromotionMessage.class);

		KStream<String, PromotionMessage> source = builder.stream("t.commodity.promotion",
				Consumed.with(stringSerde, jsonserdes));

		KStream<String, PromotionMessage> uppercase = source.mapValues(this::uppercasePromotionCode);
		
		uppercase.to("t.commodity.promotion-uppercase", Produced.with(stringSerde, jsonserdes));

		source.print(Printed.<String, PromotionMessage>toSysOut().withLabel("JSON Serde Original Stream"));
		uppercase.print(Printed.<String, PromotionMessage>toSysOut().withLabel("JSON Serde Uppercase Stream"));
